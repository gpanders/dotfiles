...
def ctcp_cb(data, modifier, modifier_data, string):
    if modifier_data != w.config_get_plugin("servers"):
        return string
...
def typed_char(data, signal, signal_data):
    ...
    if server not in w.config_get_plugin("servers").split(",") or input_s.startswith("/"):
        return w.WEECHAT_RC_OK
...
def send_typing_ctcp(nick, level):
    for server in w.config_get_plugin("servers").split(","):
        if channel_has_nick(server, w.config_get_plugin("channel"), nick):
...
if __name__ == "__main__":
    if w.register(SCRIPT_NAME, SCRIPT_AUTHOR, SCRIPT_VERSION, SCRIPT_LICENSE,
        ...
        if not w.config_get_plugin('servers'):
            w.config_set_plugin('servers', "bitlbee")
        if not w.config_get_plugin('timeout'):
            w.config_set_plugin('timeout', "4")

        w.hook_signal("input_text_changed", "typed_char", "")
        w.hook_signal(w.config_get_plugin("servers")+",irc_in_quit", "stop_typing", "")
        w.hook_signal(w.config_get_plugin("servers")+",irc_in_privmsg", "stop_typing", "")
